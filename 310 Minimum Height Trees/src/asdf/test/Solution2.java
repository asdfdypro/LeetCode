...
public class Solution2 {
    //最低树就是最长边的一半
    //先以任意点为起点，找到最长路径，再以终点为起点，找到最长路径
    //内存溢出
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        visited[0] = 1;
        path.add(0);
        search(graph, longest, visited, path, 0);
        path.clear();
        visited[0] = 0;

        //从终点出发
        int form = longest.get(longest.size() - 1);
        visited[form] = 1;
        path.add(form);
        search(graph, longest, visited, path, form);
...
//        System.out.println(Arrays.toString(longest.toArray()));

        return res;
    }


    private void search(int[][] graph, List<Integer> longest, int[] visited, List<Integer> path, int form) {
        ...
        for (int i = 0; i < graph.length; i++) {
            if (graph[form][i] > 0 && visited[i] == 0) {
                ...
                visited[i] = 1;
                path.add(i);
                search(graph, longest, visited, path, i);
                path.remove(path.size() - 1);
                visited[i] = 0;
            }

        }
        if (!isLonger && path.size() > longest.size()) {
            longest.clear();
            longest.addAll(path);
        }
    }
