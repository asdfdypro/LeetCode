...
import asdf.test.Solution2.Item;
...
public class Solution3 {
	 ...
	 */
	// 广搜 按字母搜
	// 注意词典中含有beginword的情况，此时用map记录前驱节点会有问题
	// 不能全局记录一个前驱，可能A-B-C A-D-C两条路径
	// 防止出现搜索的循环，但广搜中，不代表一次搜索后就不能再次搜索，应为同层一次搜索后不能再搜
	...
	public List<List<String>> findLadders(String beginWord, String endWord, Set<String> wordList) {
		...
		Queue<Item> rootQueue = new LinkedList<>();
		rootQueue.offer(new Item(beginWord));
		find(res, endWord, wordLength, wordList, rootQueue);
		return res;
	}
	 ...
	 *            查询队列
	 */
	private void find(List<List<String>> res, String endWord, int wordLength, Set<String> wordList,
			Queue<Item> queue) {
		...
		while (!queue.isEmpty()) {
			...
			if (wordDistance(item.word, endWord, wordLength)) {// 结束
				...
				cc = item.word.toCharArray();
				for (int i = 0; i < wordLength; i++) {
					old = cc[i];
					for (char j = 'a'; j <= 'z'; j++) {
						cc[i] = j;
						newWord = new String(cc);
						if (!item.word.equals(newWord) && wordList.contains(newWord)) {
							visited.add(newWord);
							newQueue.offer(new Item(newWord, item.list));
						}
					}
					cc[i] = old;
				}
		...
		if (newQueue != null && newQueue.size() > 0)
			find(res, endWord, wordLength, wordList, newQueue);
