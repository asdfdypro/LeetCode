...
public class Solution {
	...
	public int ladderLength(String beginWord, String endWord, Set<String> wordList) {
		...
		Queue<String> rootQueue = new LinkedList<>();
		rootQueue.offer(beginWord);
	 ...
	 *            查询队列
	 */
	private void find(int[] res, String endWord, int wordLength, Set<String> wordList,
		 Queue<String> queue) {
			...
			if (d < 2) {// 结束
				...
				for (int i = 0; i < wordLength; i++) {
					...
					for (char j = 'a'; j <= 'z'; j++) {
						...
						if (!word.equals(newWord) && wordList.contains(newWord)) {
							visited.add(newWord);
